...
execute if score seconds system matches 240 if score aliveDe playerStatus matches 1.. run tellraw @a[team=traitor] {"text":"Traitors have made poor progress. Kill more innocents or the game will end early.","color":"red"}
execute if score seconds system matches 540 if score aliveDe playerStatus >= aliveT playerStatus run tellraw @a[team=traitor] {"text":"Traitors can not initiate overtime if they don't outnumber the remaining detectives.","color":"red"}
